...
export const posts: Post[] = [
  ...
  },
  {
    ...
    year: 2025,
    date: 'Nov 21',
    description: 'OpenCode是一款在终端中运行的CLI+TUI AI编程代理工具，支持快速代码生成、调试和项目分析。本文详细介绍通过curl/npm安装OpenCode、配置MiniMax M2 API Key接入大模型，以及使用winget安装Oh My Posh美化终端显示Git状态等信息。',
  ...
  },
  {
    ...
    year: 2025,
    date: 'Nov 21',
    description: 'Unstructured是一个强大的开源文档解析库，无论PDF、Word、PPT、HTML还是图片，都可以用同一个partition()函数提取结构化数据。本文介绍其核心优势（归一化接口、语义分块、元数据保留），并提供Windows下使用Scoop安装Tesseract OCR和Poppler以支持中文PDF解析的完整指南。',
  ...
  },
  {
    ...
    year: 2025,
    date: 'Oct 30',
    description: 'Gemini CLI完整安装配置指南。包括Windows/macOS/Linux下使用nvm安装Node.js、npm全局安装@google/gemini-cli、创建Google Cloud项目并启用Gemini API、配置GOOGLE_CLOUD_PROJECT环境变量，即可开启命令行AI交互体验。',
