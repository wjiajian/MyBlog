...
2.  **环境冲突**：AI 安装的依赖或生成的构建产物可能覆盖本地配置。
3.  **终端阻塞**：Claude 执行复杂任务时，终端被占用，无法并行工作。
...
现在我们就可以可以启动 Claude 并下达指令，同时回到主目录继续我们的工作。
...
*   **主控端**：在 `my-app-main` 目录中，我们可以继续开发其他功能，或者进行 Code Review，完全不受 AI 的文件变更影响。
